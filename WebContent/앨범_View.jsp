				...
				<div class="panel-body">	
					...
					%>
						<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
							<tbody>
								<%
									Album list = queDAO.getAlbum(albumId);
								%>
								<tr>
									<td><%= list.getAlbumText() %></td>
								</tr>
								<tr>
									<td><%= list.getAlbumUserID() %></td>
								</tr>
								<tr>
									<td><img src="앨범_Read.jsp?albumId=<%=list.getAlbumId()%>" onerror="this.src='no_img.jpg'" width="300"> </td>
								</tr>
							</tbody>		
						</table>
					<a href="앨범.jsp" class="btn btn-primary">목록</a>
					<% 
						if(userID.equalsIgnoreCase(list.getAlbumUserID()))
						{
					%>
						<a href="앨범_Update.jsp?albumId=<%=list.getAlbumId() %>" class="btn btn-primary">수정</a>
						<a onClick="return confirm('정말로 삭제하겠습니까?')" href="앨범_DeleteAction.jsp?albumId=<%=list.getAlbumId() %>" class="btn btn-primary">삭제</a>
					<%
						}
					%>
</div>

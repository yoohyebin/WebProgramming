			...
			<div class="panel panel-default">    
				...
				<div class="panel-body">  
					<%
						...
						int count=bbsDAO.count(bbsID);
					%>
				
						<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
							<%
				
					BbsDTO list = bbsDAO.getBbs(bbsID);
				%>

				<tr>
					<td><%= list.getBbsTitle() %></td>
				</tr>
				<tr>
					<td><%= list.getBbsUserID() %></td>
				</tr>
				<tr>
					<td colspan="2" style="min-height: 200px; text-align:left;"><%= list.getBbsContent() %></td>
				</tr>
				<%
					if(list.getBbsFileName()!=null)
					{
				%>
				<tr>
					<td> <a href="fileDown.jsp?bbsID=<%=list.getBbsID()%>">  <%= list.getBbsFileName() %> </a></td>
				</tr>
				<%
					}
				%>
						...
						<a href="강의자료실.jsp" class="btn btn-primary">목록</a>
					<% 
						if(userID.equalsIgnoreCase(list.getBbsUserID()))
						{
					%>
						<a href="강의자료실_update.jsp?bbsID=<%=list.getBbsID()%>" class="btn btn-primary">수정</a>
						<a onClick="return confirm('정말로 삭제하겠습니까?')" href="강의자료실_deleteAction.jsp?bbsID=<%=list.getBbsID()%>" class="btn btn-primary">삭제</a>
					<%
						}
					%>
				</div>

...
<div class="center-block" style="width:900px;">
	<div class="clearfix" > 
		...
		<table class="table table-hover">
			...
			<tbody>
				<%
					...
					ArrayList<Que> list = qDAO.getList(pageNumber);
					for(int i=0; i< list.size(); i++){
					
				%>
				<tr>
					<td><%= list.get(i).getQueID() %></td>
					<td><a href="q&a_view.jsp?queID=<%=list.get(i).getQueID()%>" style="color:#444" ><%=list.get(i).getQueTitle()%></a></td>
					<td><%= list.get(i).getQueUserID() %></td>
					<td><%= list.get(i).getQueDate()%></td>
					<td><%=list.get(i).getQueCount() %></td>
				</tr>
				<%
					}
				%>
		...
		<a href="q&a_write.jsp" class="btn btn-primary pull-right">글쓰기</a>
		<br>
		<br>
		<br>
		<center>
		<form method="post" action="q&a_search.jsp">
		<table>
		<tr>
			<td>
			...
			<td><div class="form-group">             
				<input type="text" class="form-control" name="search" style="width:400px;">          
			</div> </td>
			<td>&nbsp;&nbsp;</td>
			<td><input type="submit" class="btn btn-primary" value="검색"></td>
		</tr>
		</table>
		</form>
		</center>

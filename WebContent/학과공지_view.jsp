			...
			<div class="panel panel-default">    
				...
				<div class="panel-body">
					<%
						...
						int count = nDAO.count(noticeID);
					%>
					<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
						<tbody>
							<tr>
								<td colspan="2"><%=que.getNoticeTitle().replaceAll("<","&lt").replaceAll(">","&lt").replaceAll(" ","&nbsp").replaceAll("\n","<br>") %></td>
							</tr>
							<tr>
								<td colspan="2"><%=que.getNoticeUserID().replaceAll("<","&lt").replaceAll(">","&lt").replaceAll(" ","&nbsp").replaceAll("\n","<br>") %></td>
							</tr>
							<tr>
								<td colspan="2" style="min-height: 200px; text-align:left;"><%=que.getNoticeContent().replaceAll("<","&lt").replaceAll(">","&lt").replaceAll(" ","&nbsp").replaceAll("\n","<br>")%></td>
							</tr>
						</tbody>
					</table>
					<a href="학과공지.jsp" class="btn btn-primary">목록</a>
					<% 
						if(userID.equalsIgnoreCase(que.getNoticeUserID()))
						{
					%>
						<a href="학과공지_update.jsp?noticeID=<%=que.getNoticeID()%>" class="btn btn-primary">수정</a>
						<a onClick="return confirm('정말로 삭제하겠습니까?')" href="학과공지_deleteAction.jsp?noticeID=<%=que.getNoticeID()%>" class="btn btn-primary">삭제</a>
					<%
						}
					%>

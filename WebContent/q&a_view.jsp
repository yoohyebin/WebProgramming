			...
			<div class="panel panel-default">    
				...
				<div class="panel-body">  
						...
						<%
							...
							int count = qDAO.count(queID);
						%>
						<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
							<tbody>
								<tr>
									<td colspan="2"><%=que.getQueTitle().replaceAll("<","&lt").replaceAll(">","&lt").replaceAll(" ","&nbsp").replaceAll("\n","<br>") %></td>
								</tr>
								<tr>
									<td colspan="2"><%=que.getQueUserID().replaceAll("<","&lt").replaceAll(">","&lt").replaceAll(" ","&nbsp").replaceAll("\n","<br>") %></td>
								</tr>
								<tr>
									<td colspan="2" style="min-height: 200px; text-align:left;"><%=que.getQueContent().replaceAll("<","&lt").replaceAll(">","&lt").replaceAll(" ","&nbsp").replaceAll("\n","<br>")%></td>
								</tr>
							</tbody>
						</table>
						<a href="q&a.jsp" class="btn btn-primary">목록</a>
						<% 
						if(userID.equalsIgnoreCase(que.getQueUserID()))
						{
						%>
							<a href="q&a_update.jsp?queID=<%=que.getQueID()%>" class="btn btn-primary">수정</a>
							<a onClick="return confirm('정말로 삭제하겠습니까?')" href="q&a_deleteAction.jsp?queID=<%=que.getQueID()%>" class="btn btn-primary">삭제</a>
						<%
							}
						%>
						<br>
						<br>
						<form action="q&a_reText.jsp?queID=<%=que.getQueID()%>" method="post" >
							<div>
								<textarea type="text"  class="form-control" name="reText" maxlength="100" style="height:100px;"></textarea>
								<br>
						...
						<table class="table">
							...
							<%
								...
								ArrayList<Re> list =rDAO.getList(pageNumber);
								for(int i=0; i< list.size(); i++)
								{
									if(list.get(i).getRenum()==queID)
									{
							%>
								<tr>
				                    <td><%=list.get(i).getReUserID() %></td>
				                    <td><%=list.get(i).getReText() %></td>
				                    <td><%=list.get(i).getReDate() %></td>
				                  <% if(userID.equals(list.get(i).getReUserID())){ %>
				                    <td><a href="q&a_reupdate.jsp?reID=<%=list.get(i).getReID()%>" class="btn btn-primary">수정</a></td>
				                    <td><a onClick="return confirm('정말로 삭제하겠습니까?')" href="q&a_redeleteAction.jsp?reID=<%=list.get(i).getReID()%>" class="btn btn-primary">삭제</a></td>
				                   <%} %>
				                </tr>
				                <%
									}
